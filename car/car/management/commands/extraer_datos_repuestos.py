    ...
    def extraer_datos_descripcion(self, descripcion):
        ...
        for pattern in marca_patterns:
            ...
            if match:
                ...
                # Evitar marcas que contengan palabras que no son marcas
                if len(marca) > 2 and not any(word in marca.lower() for word in ['tecnología', 'mineral', 'sintético', 'norma', 'api', 'compatible', 'vehículos', 'diesel', 'bencineros', 'formato', 'color', 'punto', 'congelamiento', 'ebullición', 'con', 'detergente', 'capacidad', 'mezcla', 'hasta', 'pegamento', 'contacto', 'renovador', 'gomas', 'espuma', 'silicona', 'tableros', 'aroma', 'original']):
                    datos['marca'] = marca
                break
        ...
        for pattern in codigo_patterns:
            ...
            if match:
                ...
                # Evitar códigos que sean solo "Marca" o palabras similares
                if len(codigo) > 1 and codigo.lower() not in ['marca', 'vehículo', 'vehículos']:
