#!/usr/bin/env python3
"""
Script para probar la búsqueda corregida de repuestos en compras
"""
...
def probar_busqueda_compras_corregida():
    print("🔍 PROBANDO BÚSQUEDA CORREGIDA EN COMPRAS")
    print("=" * 60)
    ...
    try:
        # 1. BUSCAR ACEITE 10W-40 CON FILTROS CORREGIDOS
        print("📊 BUSCANDO ACEITE 10W-40 CON FILTROS CORREGIDOS...")
        query = "Aceite 10w-40"
        
        cursor.execute("""
            SELECT id, nombre, sku, marca, oem, referencia, origen_repuesto, marca_veh, tipo_de_motor, precio_costo, precio_venta, stock
            FROM car_repuesto 
            WHERE (
                nombre LIKE ? OR
                sku LIKE ? OR
                ...
                (marca IS NULL OR marca = '' OR marca NOT IN ('general', '')) AND
                (origen_repuesto IS NULL OR origen_repuesto = '' OR origen_repuesto NOT IN ('sin-origen', '')) AND
                (marca_veh IS NULL OR marca_veh = '' OR marca_veh NOT IN ('xxx', 'xxxx', '')) AND
                (tipo_de_motor IS NULL OR tipo_de_motor = '' OR tipo_de_motor NOT IN ('zzzzzz', 'zzzz', ''))
            )
            ORDER BY nombre
            LIMIT 10
        ...
        for repuesto in repuestos:
            r_id, nombre, sku, marca, oem, referencia, origen, marca_veh, tipo_motor, precio_costo, precio_venta, stock = repuesto
            print(f"    ID: {r_id}")
        ...
        print("\n🎯 RESUMEN DE LA BÚSQUEDA CORREGIDA EN COMPRAS:")
        ...
        print("\n🎉 ¡BÚSQUEDA EN COMPRAS CORREGIDA EXITOSAMENTE!")
...
if __name__ == "__main__":
    probar_busqueda_compras_corregida()
